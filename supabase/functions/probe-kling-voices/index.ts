      ...
      if (videoUrl) {
        try {
          ...
          if (!vr.ok) {
            ...
            const { error: upErr } = await svc.storage.from("generated-videos").upload(path, bytes, { contentType: "video/mp4", upsert: true });
            if (upErr) err = `upload: ${upErr.message}`;
            else { const { data: sg } = await svc.storage.from("generated-videos").createSignedUrl(path, 86400); publicUrl = sg?.signedUrl || null; }
          }
