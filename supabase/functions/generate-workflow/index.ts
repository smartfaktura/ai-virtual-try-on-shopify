...
2. Use [PRODUCT IMAGE${additionalProducts?.length ? ' 1' : ''}] as the product source of truth — preserve the same shape, proportions, colors, materials, texture, finish, structure, and visible details without redesigning it.${additionalProducts?.length ? `\n${additionalProducts.map((_, idx) => `${idx + 3}. Use [PRODUCT IMAGE ${idx + 2}] as the product source of truth — preserve the same shape, proportions, colors, materials, texture, finish, structure, and visible details without redesigning it.`).join('\n')}\n${additionalProducts.length + 3}. The final image MUST show exactly ${1 + additionalProducts.length} distinct products. Count them before finalizing. Missing any product is a failure.` : ''}
