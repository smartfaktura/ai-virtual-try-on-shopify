...
2. Use [PRODUCT IMAGE${additionalProducts?.length ? ' 1' : ''}] as the product source of truth — preserve EXACT shape, silhouette, proportions, real-world dimensions, colors, materials, texture, finish, hardware, stitching, and seams. Do NOT redesign, restyle, or "improve" the product. The output product must be visually interchangeable with the reference.${additionalProducts?.length ? `\n${additionalProducts.map((_, idx) => `${idx + 3}. Use [PRODUCT IMAGE ${idx + 2}] as the product source of truth — preserve EXACT shape, silhouette, proportions, real-world dimensions, colors, materials, texture, finish, hardware, stitching, and seams. Do NOT redesign, restyle, or "improve" the product. The output product must be visually interchangeable with the reference.`).join('\n')}\n${additionalProducts.length + 3}. The final image MUST show exactly ${1 + additionalProducts.length} distinct products. Count them before finalizing. Missing any product is a failure.` : ''}
...
7. PRODUCT FIDELITY LOCK (CRITICAL): Any logo, brand mark, printed text, label, monogram, embroidery, tag, or graphic visible on [PRODUCT IMAGE] MUST be reproduced character-for-character in the same position, size, color, and finish. Do NOT invent, translate, restyle, abbreviate, or substitute any letter, number, or symbol. If the reference logo is tone-on-tone, embossed, or debossed, keep it tone-on-tone — do NOT print it in a contrasting color. If unsure of a character, keep the original glyph exactly as shown.
