...
// Webhook handler — sends auth emails directly via Resend API
async function handleWebhook(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!resendRes.ok) {
      console.error('Resend API error', { status: resendRes.status, body: resendBody, run_id, emailType })
      await supabase.from('email_send_log').insert({
        message_id: messageId,
        template_name: emailType,
        recipient_email: payload.data.email,
        status: 'failed',
        error_message: `Resend ${resendRes.status}: ${JSON.stringify(resendBody)}`,
      })
    ...
    console.log('Auth email sent via Resend', { emailType, email: payload.data.email, run_id, resendId: resendBody.id })
    ...
    console.error('Failed to send via Resend', { error: sendError, run_id, emailType })
    await supabase.from('email_send_log').insert({
      message_id: messageId,
      template_name: emailType,
      recipient_email: payload.data.email,
