...
export default function Generate() {
  ...
  const workflowCostPerImage = quality === 'high' ? 8 : 4;
  const singleProductCreditCost = isUpscale ? 0 : (generationMode === 'virtual-try-on' ? parseInt(imageCount) * 8 * tryOnSceneCount : (hasWorkflowConfig ? workflowImageCount * workflowCostPerImage : parseInt(imageCount) * 8 * tryOnSceneCount));
              ...
              activeWorkflow?.uses_tryon ? (() => {
                ...
                <div className="space-y-3">
                  ...
                  {(() => {
                    ...
                    if (filteredSamples.length === 0 && searchQuery) {
                      return <p className="text-center text-sm text-muted-foreground py-6">No products match "{searchQuery}"</p>;
                    }

                    if (productViewMode === 'list') {
                      return (
                        <div className="space-y-1 max-h-[420px] overflow-y-auto pr-1">
                          {filteredSamples.map(sp => {
                            const isSelected = selectedProductIds.has(sp.id);
                            const isDisabled = !isSelected && selectedProductIds.size >= MAX_PRODUCTS_PER_BATCH;
                            const imgUrl = sp.images[0]?.url;
                            return (
                              <button
                                key={sp.id}
                                type="button"
                                onClick={() => {
                                  ...
                                  if (newSet.has(sp.id)) { newSet.delete(sp.id); }
                                  else if (newSet.size < MAX_PRODUCTS_PER_BATCH) { newSet.add(sp.id); }
                              ...
                              >
                                {imgUrl ? (
                                  <img src={imgUrl} alt={sp.title} className="w-10 h-10 rounded-md object-cover flex-shrink-0" />
                                ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-xs font-medium text-foreground truncate">{sp.title}</p>
                                  {sp.productType && <p className="text-[10px] text-muted-foreground truncate">{sp.productType}</p>}
                                </div>
                                <div className={cn(
                    ...
                    return (
                      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
                        {filteredSamples.map(sp => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {imgUrl ? (
                                <img src={imgUrl} alt={sp.title} className="w-full aspect-square object-cover rounded-t-md" />
                              ) : (
                                <div className="w-full aspect-square bg-muted flex items-center justify-center rounded-t-md">
                                  <Package className="w-6 h-6 text-muted-foreground" />
                                </div>
                              )}
                              <div className="px-1.5 py-1.5 bg-card">
                                <p className="text-[10px] font-medium text-foreground leading-tight line-clamp-2">{sp.title}</p>
                                {sp.productType && (
                                  <p className="text-[9px] text-muted-foreground truncate mt-0.5">{sp.productType}</p>
                                )}
                              </div>
                            </button>
                          );
                        ...
                        >
                          ...
                          <span className="text-[10px] font-medium">Upload Product</span>
                        </button>
                      </div>
                    );
                  })()}
