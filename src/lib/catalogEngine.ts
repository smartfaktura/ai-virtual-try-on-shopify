...
export function assemblePrompt(input: PromptAssemblyInput): string {
  ...
  if (isAnchorShot) {
    ...
    // DERIVATIVE ON-MODEL: single-reference system — ONLY the model identity image is sent
    // Outfit consistency is enforced purely via text (no anchor/product images to dilute face signal)
    prompt += '\nIMAGE ROLE ASSIGNMENT:';
    prompt += '\n- Image 1 (MODEL IDENTITY): This is the SOLE IDENTITY SOURCE. The output person must be THIS EXACT person — same face, facial structure, jawline, cheekbones, nose shape, eye shape, eye color, eyebrows, lip shape, skin tone, skin texture, hair color, hair style, hair length, and body proportions. There is NO other image reference — this is the only visual input.';
    prompt += '\nFACE LOCK: Since Image 1 is the ONLY reference image, replicate this person\'s identity with maximum fidelity. Do NOT invent a new face. Do NOT blend with any other appearance. The output face must be a pixel-perfect match to Image 1.';
    ...
    prompt += `\nOUTFIT FROM TEXT: The model's outfit is defined ENTIRELY by the text prompt above. Follow the wardrobe description exactly: ${effectiveWardrobe}. The hero product is "${productTitle}". Do NOT deviate from the described clothing. Do NOT invent additional accessories or items not mentioned.`;
