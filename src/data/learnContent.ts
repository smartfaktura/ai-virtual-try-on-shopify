...
export const LEARN_GUIDES: LearnGuide[] = [
  // ───────────────────────────────────────────── VISUAL STUDIO ─────────────────────────────────────────────
  {
    ...
    bestFor: [
      'Refreshing low-res catalog photos before a launch',
      'Single-product hero images for Meta or TikTok ads',
      'Editorial campaign frames without booking a studio',
    ],
    whatYouNeed: [
      'One sharp front-facing product photo, 1024px+ — no harsh shadows, no watermarks',
      'Optional: a Brand Profile to lock palette and mood across runs',
    ],
    whatYouGet: [
      '2K lossless PNGs, web and print ready',
      '2–4 variations per scene to compare',
      'Per-scene aspect ratios — not one fixed format',
    ],
    quickStart: [
      { label: 'Product', detail: 'Add or pick a product. Wait for "Analysis complete" before moving on.' },
      { label: 'Shots', detail: 'Browse scenes by category. Start with 2–3 to compare cleanly.' },
      { label: 'Setup', detail: 'Optionally lock a model and Brand Profile. Skip = neutral defaults.' },
      { label: 'Generate', detail: 'Confirm credits and hit Generate. First image lands in ~30s.' },
    ],
    tips: [
      { type: 'do', text: 'Use the highest-res source you have — VOVV preserves detail it can see.' },
      { type: 'do', text: 'Lock a Brand Profile before a multi-scene run for consistency.' },
      { type: 'dont', text: 'Do not upload screenshots, mockups or watermarked photos.' },
      { type: 'dont', text: 'Do not mix unrelated SKUs in one run — split into separate generations.' },
    ],
