...
function StepVisual({ index }: { index: number }) {
  if (index === 0) {
    return (
      <div className="w-full aspect-[3/2] rounded-3xl bg-white border border-[#e8e7e4] shadow-sm p-6 sm:p-8 flex items-center justify-center">
        <div className="w-full h-full rounded-2xl border-2 border-dashed border-[#d4d4d4] flex flex-col items-center justify-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#f5f5f3] flex items-center justify-center">
            <Upload size={26} className="text-[#94a3b8]" />
          </div>
          <div className="space-y-1.5 text-center">
            <div className="h-2.5 w-32 rounded-full bg-[#e8e7e4] mx-auto" />
            <div className="h-2 w-24 rounded-full bg-[#f0efed] mx-auto" />
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full aspect-[3/2] rounded-3xl bg-white border border-[#e8e7e4] shadow-sm p-5 sm:p-6 flex flex-col gap-3">
        ...
        <div className="flex-1 grid grid-cols-3 gap-3">
          {['from-amber-100 to-orange-50', 'from-rose-100 to-pink-50', 'from-sky-100 to-blue-50'].map((c, i) => (
            <div
              key={i}
              className={`rounded-xl bg-gradient-to-br ${c} border-2 ${i === 0 ? 'border-[#1a1a2e]' : 'border-transparent'} flex flex-col items-center justify-center gap-2 p-3`}
            >
              <Image size={20} className="text-[#94a3b8]" />
              <div className="h-1.5 w-12 rounded-full bg-[#1a1a2e]/10" />
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 grid grid-cols-2 gap-3">
        {['from-amber-50 to-orange-50', 'from-rose-50 to-pink-50', 'from-emerald-50 to-teal-50', 'from-sky-50 to-blue-50'].map((c, i) => (
          <div key={i} className={`rounded-xl bg-gradient-to-br ${c} border border-white/60 shadow-inner flex items-center justify-center`}>
            <div className="w-10 h-16 rounded-lg bg-white/40 shadow-inner" />
          </div>
        ))}
      </div>
    </div>
        ...
        <div ref={ref} className="space-y-16 lg:space-y-28">
          {steps.map((step, i) => {
            ...
            return (
              ...
              >
                {/* On mobile: visual first, then text */}
