          ...
          <div className="flex flex-col gap-1.5 mb-1">
            {sceneThumb && sceneName && (
              <div className="flex items-center gap-2">
                <img src={getOptimizedUrl(sceneThumb, { width: 56, quality: 60 })} alt={sceneName} className="w-7 h-7 rounded-md object-contain bg-black/20" />
                <span className="text-white/90 text-[11px] font-medium truncate">{sceneName}</span>
              </div>
            )}
            {modelThumb && modelName && (
              <div className="flex items-center gap-2">
                <img src={getOptimizedUrl(modelThumb, { width: 56, quality: 60 })} alt={modelName} className="w-7 h-7 rounded-md object-contain bg-black/20" />
