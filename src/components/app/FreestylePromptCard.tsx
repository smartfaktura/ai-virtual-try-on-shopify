...
const MODEL_AVATARS = [
  { name: 'Zara', src: getOptimizedUrl(getLandingAssetUrl('models/model-female-athletic-mixed.jpg'), { quality: 40 }) },
  { name: 'Freya', src: getOptimizedUrl(getLandingAssetUrl('models/model-female-average-nordic.jpg'), { quality: 40 }) },
  { name: 'Olivia', src: getOptimizedUrl(getLandingAssetUrl('models/model-035-olivia.jpg'), { quality: 40 }) },
];

const CHIP_THUMB = getOptimizedUrl(getLandingAssetUrl('showcase/source-crop-top.jpg'), { quality: 40 });
const SCENE_THUMB = getOptimizedUrl(getLandingAssetUrl('showcase/cafe-lifestyle.png'), { quality: 40 });

const RESULT_CARDS = [
  { label: 'Studio', src: getOptimizedUrl(getLandingAssetUrl('showcase/virtual-tryon-1.png'), { quality: 50 }) },
  { label: 'Court', src: getOptimizedUrl(getLandingAssetUrl('showcase/virtual-tryon-2.png'), { quality: 50 }) },
  { label: 'Café', src: getOptimizedUrl(getLandingAssetUrl('showcase/cafe-lifestyle.png'), { quality: 50 }) },
];
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <div className="absolute inset-0 flex flex-col px-2.5 pt-3 pb-2 gap-1.5">
          {/* Prompt box */}
          <div className={cn(
            'rounded-lg border bg-card/80 backdrop-blur-sm px-2.5 py-2 min-h-[32px] flex items-start transition-colors duration-300',
            hovered ? 'border-primary/30' : 'border-border/50',
          )}>
            <p className="text-[10px] leading-relaxed text-foreground/80 flex-1 line-clamp-2">
              {typedText}
          ...
          <div className="flex items-center gap-1 overflow-hidden">
            {CHIPS.map((chip) => {
              ...
              return (
                <div
                  key={chip.key}
                  className={cn(
                    'inline-flex items-center gap-0.5 h-[22px] px-1.5 rounded-full text-[8px] font-medium border whitespace-nowrap transition-all duration-500 shrink-0',
                    active
                    ...
                    <div className="flex -space-x-1 shrink-0">
                      {MODEL_AVATARS.map((m, idx) => (
                        <img key={m.name} src={m.src} alt={m.name}
                          className="w-3.5 h-3.5 rounded-full object-cover ring-1 ring-card"
                          style={{ zIndex: MODEL_AVATARS.length - idx }}
                        />
                      ))}
                    </div>
                  ) : active && chip.key === 'product' ? (
                    <img src={CHIP_THUMB} alt="Product" className="w-3.5 h-3.5 object-cover rounded ring-1 ring-border/40 shrink-0" />
                  ) : active && chip.key === 'scene' ? (
                    <img src={SCENE_THUMB} alt="Scene" className="w-3.5 h-3.5 object-cover rounded ring-1 ring-border/40 shrink-0" />
                  ) : (
            ...
            <div className={cn(
              'ml-auto w-[22px] h-[22px] rounded-md flex items-center justify-center transition-all duration-500 shrink-0',
              generating
              ...
              {generating ? (
                <div className="w-2.5 h-2.5 border border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />
              ) : (
          ...
          {/* Result images — fill remaining space */}
          <div className="flex-1 grid grid-cols-3 gap-1.5 min-h-0">
            {RESULT_CARDS.map((card, i) => {
              const visible = visibleResults.includes(i);
              return (
                <div
                  key={i}
                  className={cn(
                    ...
                    showResults
                      ? visible
                        ? 'border-border/40 opacity-100 translate-y-0 scale-100'
                        : 'border-border/40 opacity-0 translate-y-2 scale-95'
                      : 'border-border/20 bg-muted/20',
                  )}
                >
                  {/* Placeholder skeleton — always visible underneath */}
                  <div className="absolute inset-0 bg-gradient-to-br from-muted/30 to-muted/10" />
                  <div className="absolute bottom-1 left-1 right-1 h-1.5 rounded-full bg-muted/30" />
                  ...
                  {/* Label overlay */}
                  {visible && (
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/50 to-transparent pt-3 pb-1 px-1.5">
                      <span className="text-[7px] font-semibold text-white/90 tracking-wide uppercase">{card.label}</span>
                    </div>
                  )}
                </div>
              );
