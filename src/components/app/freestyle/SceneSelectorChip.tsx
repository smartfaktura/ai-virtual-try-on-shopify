      ...
      {visibleCategories.map(cat => {
        ...
        return (
          <div key={cat}>
            ...
            <div className={cn('grid gap-1.5', expanded ? `gap-2` : isMobile ? 'grid-cols-3' : 'grid-cols-3 lg:gap-2')} style={expanded ? { gridTemplateColumns: `repeat(${expandedColumns}, minmax(0, 1fr))` } : undefined}>
              {poses.map(pose => (
                <button
                  key={pose.poseId}
