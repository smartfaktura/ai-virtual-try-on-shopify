          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-sm font-medium text-foreground">Create your own brand model</p>
                <p className="text-[11px] text-muted-foreground">Generate via wizard or from your reference image</p>
              </div>
            </div>
          </button>
        )}
...
function PropPickerModal({ open, onOpenChange, products, excludeIds, alreadySelected, onConfirm }: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl w-[92vw] max-h-[85vh] flex flex-col">
                ...
                <CardContent className="p-4 sm:p-5 space-y-4 sm:space-y-5">
                  ...
                  <Collapsible className="rounded-xl border border-border/60 bg-muted/20 overflow-hidden">
                    <CollapsibleTrigger className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-muted/40 transition-colors cursor-pointer group/note focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 [&[data-state=open]>svg:last-child]:rotate-180">
                      ...
                      {details.customOutfitNote && (
                        <span className="text-[10px] text-muted-foreground/60 truncate ml-1 flex-1 text-left max-w-[120px] sm:max-w-[260px]">{details.customOutfitNote}</span>
                      )}
                      <ChevronDown className="w-3.5 h-3.5 text-muted-foreground ml-auto transition-transform flex-shrink-0" />
                    </CollapsibleTrigger>
                    <CollapsibleContent>
                 ...
                 <div className="space-y-3">
                   {selectedProductsList.map((product) => {
                     ...
                     const productResolution = resolveOutfitConflicts(analysis?.category || primaryCategory, analysis?.garmentType);
                     const lockedSlotLabel = productResolution.lockedSlot
                           ...
                           <div className="flex-1 min-w-0">
                             ...
                             <div className="flex items-center gap-1.5 mt-0.5">
                               ...
                               <span className="text-[10px] text-muted-foreground">
                                 {productModelShots.length} shot{productModelShots.length !== 1 ? 's' : ''}
                               </span>
                             </div>
                           </div>
                         </div>
