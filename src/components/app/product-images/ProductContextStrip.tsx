...
export function ProductContextStrip({ products, onChangeProducts }: ProductContextStripProps) {
  ...
  return (
    <TooltipProvider>
      <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-muted/50 border border-border/60 min-w-0 max-w-full overflow-hidden">
            ...
            <Tooltip key={p.id}>
              <TooltipTrigger asChild>
                <div className="w-8 h-8 rounded-md overflow-hidden bg-white flex-shrink-0 border border-border/40">
                  <ShimmerImage src={getOptimizedUrl(p.image_url, { quality: 60 })} alt={p.title} loading="eager" className="w-full h-full object-cover" />
                </div>
              </TooltipTrigger>
