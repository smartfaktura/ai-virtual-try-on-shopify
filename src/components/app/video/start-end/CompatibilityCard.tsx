  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-2">
      <div className="flex items-center gap-2">
        <span className={cn('inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-xs font-medium', meta.tone)}>
          <Icon className="h-3 w-3" />
          {meta.label}
        </span>
        <span className="text-xs text-muted-foreground">Frame compatibility</span>
      </div>
      <p className="text-sm text-foreground">{compatibility.reason}</p>
      {compatibility.recommendation && (
        <p className="text-xs text-muted-foreground">{compatibility.recommendation}</p>
      )}
      {compatibility.sharedElements.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-1">
          {compatibility.sharedElements.slice(0, 6).map((el) => (
            <span key={el} className="px-2 py-0.5 rounded-md bg-muted text-[11px] text-muted-foreground">
              {el}
            </span>
          ))}
        </div>
