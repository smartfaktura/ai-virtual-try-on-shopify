...
import { TemplatePreviewCard, getTemplateImage } from './TemplatePreviewCard';
...
export function BulkSettingsModal({
  open,
  onClose,
  onConfirm,
  selectedProducts,
  ...
  const selectedPose = poses.find(p => p.poseId === poseId);
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Bulk Generation Settings"
      primaryAction={{
        content: `Generate ${productCount * imageCount} Images`,
          ...
          {mode === 'product-only' ? (
            ...
            <BlockStack gap="400">
              <BlockStack gap="300">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-[150px] overflow-y-auto">
                  {models.map(model => (
                    <div
                      key={model.modelId}
