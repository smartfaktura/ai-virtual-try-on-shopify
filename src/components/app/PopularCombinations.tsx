...
export function PopularCombinations({ combinations, onSelect }: PopularCombinationsProps) {
  if (combinations.length === 0) return null;
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h3 className="font-bold text-sm">Quick Start</h3>
        <Badge variant="secondary" className="text-[10px]">Popular Looks</Badge>
      </div>
          ...
          <Card key={combo.id} onClick={() => onSelect(combo.model, combo.pose)} className="p-3 cursor-pointer transition-all duration-200 hover:shadow-md hover:border-primary/50 group">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{combo.model.name} • {combo.pose.name}</p>
                <p className="text-[10px] text-muted-foreground">{combo.bestFor}</p>
              </div>
              <span className="opacity-0 group-hover:opacity-100 transition-opacity text-xs font-medium text-primary">Select →</span>
            </div>
          </Card>
        ))}
      </div>
...
export function createPopularCombinations(models: ModelProfile[], poses: TryOnPose[]): PopularCombination[] {
  ...
  if (yuki && studioFront) combinations.push({ id: 'combo_1', model: yuki, pose: studioFront, label: 'Classic Lookbook', bestFor: 'E-commerce product pages' });
  if (amara && lifestyle) combinations.push({ id: 'combo_2', model: amara, pose: lifestyle, label: 'Lifestyle Casual', bestFor: 'Social media & ads' });
  if (marcus && streetwear) combinations.push({ id: 'combo_3', model: marcus, pose: streetwear, label: 'Urban Street', bestFor: 'Streetwear brands' });
  if (isabella && editorial) combinations.push({ id: 'combo_4', model: isabella, pose: editorial, label: 'Editorial Plus', bestFor: 'Inclusive campaigns' });
  return combinations;
}
