...
export function MetricCard({ title, value, suffix, icon: Icon, tooltip, trend, loading, onClick, progress, progressColor, action, description, popoverAlign }: MetricCardProps) {
  ...
  const cardContent = (
    <div
      className={`rounded-xl border border-border/40 bg-card p-4 sm:p-5 h-[140px] flex flex-col justify-between transition-all duration-200 hover:border-border/80 hover:shadow-sm ${onClick ? 'cursor-pointer' : ''}`}
      onClick={onClick}
    ...
    >
      <div>
        ...
        {value !== undefined ? (
          <div className="mt-2">
            <p className="text-xl font-bold text-foreground tracking-tight leading-none whitespace-nowrap">{value}</p>
            {suffix && (
