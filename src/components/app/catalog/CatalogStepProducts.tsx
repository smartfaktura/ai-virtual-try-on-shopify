...
import { ChevronRight, Package, Search, LayoutGrid, List, Check, Globe, Upload, Download, Plus } from 'lucide-react';
    ...
    <div className="space-y-5">
      ...
      <Tabs defaultValue="library" className="w-full">
        ...
        <TabsContent value="library">
          {products.length === 0 ? (
            <div className="text-center py-16 rounded-xl border-2 border-dashed border-border bg-muted/30">
              <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center mx-auto mb-3">
                <Package className="w-6 h-6 text-muted-foreground/50" />
              </div>
              <p className="text-sm font-medium text-foreground">No products yet</p>
              <p className="text-xs text-muted-foreground mt-1 mb-4">Add products first to get started</p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 items-center">
                ...
                {selectedProductIds.size > 0 && (
                  <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => onProductSelectionChange(new Set())}>Clear</Button>
                )}
              ...
              {selectedProductIds.size > 0 && (
                <div className="flex gap-2 items-center">
                  ...
                  <span className="text-[10px] text-muted-foreground">(max {maxProducts})</span>
                </div>
              )}

              {filtered.length === 0 && searchQuery && (
                <p className="text-center text-sm text-muted-foreground py-8">No products match "{searchQuery}"</p>
              )}
                ...
                <div className="space-y-1 max-h-[420px] overflow-y-auto pr-1">
                  {visible.map(up => {
                    ...
                    return (
                      ...
                      >
                        <ShimmerImage src={getOptimizedUrl(up.image_url, { quality: 60 })} alt={up.title} className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                        ...
                        <div className={cn(
                          ...
                          isSelected ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/20'
                        )}>
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {visible.map(up => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isDisabled}
                          className={cn(
                            'group relative flex flex-col rounded-xl overflow-hidden transition-all duration-150 text-left',
                            isSelected
                        ...
                        >
                          ...
                          <div className={cn(
                            ...
                            isSelected
                              ...
                              : 'border-white/80 bg-white/40 backdrop-blur-sm opacity-0 group-hover:opacity-100'
                          )}>
                            {isSelected && <Check className="w-3 h-3" />}
                          </div>
                          <ShimmerImage src={getOptimizedUrl(up.image_url, { quality: 60 })} alt={up.title} className="w-full aspect-square object-cover" />
                          <div className="px-2.5 py-2 bg-card">
                            <p className="text-[11px] font-medium text-foreground leading-tight line-clamp-2">{up.title}</p>
                            {up.product_type && (
                    ...
                    <button
                      ...
                      className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-border hover:border-primary/30 hover:bg-muted/30 transition-all duration-150 aspect-square text-muted-foreground"
                    >
        ...
        <TabsContent value="csv">
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <div className="rounded-xl border-2 border-dashed border-border hover:border-primary/30 transition-colors p-10 text-center cursor-pointer">
