...
export function ContextualFeedbackCard({
  ...
  if (step === 'success') {
    return (
      <div className={cn('flex justify-center animate-in fade-in slide-in-from-bottom-2 duration-300', className)}>
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-primary text-primary-foreground shadow-lg text-xs font-medium">
  ...
  if (step === 'step1') {
    return (
      <div className={cn('flex justify-center animate-in fade-in slide-in-from-bottom-2 duration-300', className)}>
        <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-primary text-primary-foreground shadow-lg">
          <MessageSquare className="w-3.5 h-3.5 shrink-0" />
          <span className="text-xs font-medium whitespace-nowrap">{questionText}</span>
          <div className="flex items-center gap-1">
            {(['yes', 'almost', 'no'] as const).map(key => (
              <button
                key={key}
                onClick={() => handleAnswer(key)}
                className="px-2.5 py-1 rounded-full text-[11px] font-medium border border-primary-foreground/30 text-primary-foreground/80 hover:bg-primary-foreground/10 hover:text-primary-foreground transition-colors"
              >
                {buttonLabels[key]}
              </button>
            ))}
          ...
          <button
            onClick={dismiss}
            className="ml-0.5 text-primary-foreground opacity-70 hover:opacity-100 transition-opacity shrink-0"
          >
            <X className="w-3 h-3" />
          </button>
        </div>
