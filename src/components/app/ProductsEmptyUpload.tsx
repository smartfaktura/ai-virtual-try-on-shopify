...
export function ProductsEmptyUpload({ onFilesSelected, onMethodSelect }: ProductsEmptyUploadProps) {
  ...
  const visibleMethods = isMobile ? METHODS.filter(m => m.id !== 'paste') : METHODS;
  ...
  return (
    <div className="rounded-2xl border bg-card p-6 sm:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-[1.4fr_1fr]">
        {/* Primary surface */}
        {isMobile ? (
          <div className="flex flex-col items-center justify-center text-center rounded-xl border bg-muted/20 px-5 py-8 min-h-[180px]">
            ...
            <p className="text-[11px] text-muted-foreground/70 mt-3">JPG, PNG, WEBP</p>
            <input
              ref={inputRef}
              type="file"
              multiple
        ...
        <div className="flex flex-col">
          ...
          <div className="rounded-xl border bg-background/50 divide-y overflow-hidden">
            {visibleMethods.map(({ id, label, sub, icon: Icon }) => (
              <button
                key={id}
                type="button"
