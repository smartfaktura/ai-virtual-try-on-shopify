...
import type { TechModuleAnswers } from "./schema";
import { Block, ModuleChip, SmallField } from "../_shared/QuestionsScaffold";

type Answers = Partial<TechModuleAnswers>;
...
export function TechQuestions({ value, onChange }: Props) {
  const v: Answers = {
  ...
  const toggleCamera = (c: string) => {
    ...
    const next = cur.includes(c as never)
      ? cur.filter((x) => x !== c)
      : cur.length < TECH_MAX_CAMERA_FEELS
        ? [...cur, c]
        : cur;
    ...
    <div className="space-y-8">
      <Block label="Archetype" required>
        <div className="grid grid-cols-2 gap-2">
          {TECH_ARCHETYPES.map((a) => (
            <ModuleChip
              key={a.value}
              active={v.archetype === a.value}
              onClick={() =>
                onChange({ archetype: a.value as TechArchetype })
              }
            >
              {a.label}
            </ModuleChip>
      ...
      <Block label="Product type" required>
        <div className="flex flex-wrap gap-2">
          {TECH_PRODUCT_TYPES.map((t) => (
            <ModuleChip
              key={t.value}
              active={v.product_type === t.value}
              onClick={() =>
                onChange({ product_type: t.value as TechProductType })
              }
            >
              {t.label}
            </ModuleChip>
      ...
      <Block label="Presentation" required>
        <div className="grid grid-cols-2 gap-2">
          {TECH_PRESENTATIONS.map((p) => (
            <ModuleChip
              key={p.value}
              active={v.presentation === p.value}
              onClick={() => {
                const next: Answers = {
                  presentation: p.value as TechPresentation,
                };
        ...
        <div className="space-y-3">
          <SmallField label="Color anchor">
            <Input
              ...
              placeholder="e.g. graphite, brushed aluminium"
            />
          </SmallField>
          <SmallField
            label="Camera feel"
            hint={`${(v.finishing?.camera_feel ?? []).length}/${TECH_MAX_CAMERA_FEELS}`}
          >
            <div className="flex flex-wrap gap-2">
              {TECH_CAMERA_FEELS.map((c) => (
                <ModuleChip
                  key={c}
