...
import type { HomeModuleAnswers } from "./schema";
import { Block, ModuleChip, SmallField } from "../_shared/QuestionsScaffold";

type Answers = Partial<HomeModuleAnswers>;
...
export function HomeQuestions({ value, onChange }: Props) {
  const v: Answers = {
  ...
  const toggleCamera = (c: string) => {
    ...
    const next = cur.includes(c as never)
      ? cur.filter((x) => x !== c)
      : cur.length < HOME_MAX_CAMERA_FEELS
        ? [...cur, c]
        : cur;
    ...
    <div className="space-y-8">
      <Block label="Archetype" required>
        <div className="grid grid-cols-2 gap-2">
          {HOME_ARCHETYPES.map((a) => (
            <ModuleChip
              key={a.value}
              active={v.archetype === a.value}
              onClick={() =>
                onChange({ archetype: a.value as HomeArchetype })
              }
            >
              {a.label}
            </ModuleChip>
        ...
        <div className="flex flex-wrap gap-2">
          {HOME_PRODUCT_TYPES.map((t) => (
            <ModuleChip
              ...
              onClick={() =>
                onChange({ product_type: t.value as HomeProductType })
              }
            >
              {t.label}
            </ModuleChip>
      ...
      <Block label="Presentation" required>
        <div className="grid grid-cols-2 gap-2">
          {HOME_PRESENTATIONS.map((p) => (
            <ModuleChip
              key={p.value}
              active={v.presentation === p.value}
              onClick={() => {
                const next: Answers = {
                  presentation: p.value as HomePresentation,
                };
        ...
        <div className="space-y-3">
          <SmallField label="Color anchor">
            <Input
              ...
              placeholder="e.g. warm oak, bone white"
            />
          </SmallField>
          <SmallField
            label="Camera feel"
            hint={`${(v.finishing?.camera_feel ?? []).length}/${HOME_MAX_CAMERA_FEELS}`}
          >
            <div className="flex flex-wrap gap-2">
              {HOME_CAMERA_FEELS.map((c) => (
                <ModuleChip
                  key={c}
