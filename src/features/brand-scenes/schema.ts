...
export const brandSceneBaseAnswersSchema = z
  .object({
    ...
    lens_custom: z.string().trim().min(1).max(120).optional(),
    depth_of_field: sceneDofSchema.optional(),
    depth_of_field_custom: z.string().trim().min(1).max(120).optional(),
    ...
    finish_custom: z.string().trim().min(1).max(120).optional(),
    avoid: z.string().trim().max(240).optional(),
    subject_focus: subjectFocusSchema.optional(),
    subject_focus_custom: z.string().trim().min(1).max(120).optional(),
    surface: surfaceSchema.optional(),
    prop_density: z.number().int().min(0).max(4).optional(),
    color_contrast: colorContrastSchema.optional(),
    color_contrast_custom: z.string().trim().min(1).max(120).optional(),
    saturation: saturationSchema.optional(),
    saturation_custom: z.string().trim().min(1).max(120).optional(),
    shadows: shadowSchema.optional(),
    shadows_custom: z.string().trim().min(1).max(120).optional(),
    composition: compositionSchema.optional(),
    composition_custom: z.string().trim().min(1).max(120).optional(),
    negative_space_intent: negSpaceIntentSchema.optional(),
    negative_space_intent_custom: z.string().trim().min(1).max(120).optional(),
    aesthetic_era: aestheticEraSchema.optional(),
    realism: realismSchema.optional(),
    realism_custom: z.string().trim().min(1).max(120).optional(),
