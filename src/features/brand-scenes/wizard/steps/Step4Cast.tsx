      ...
      {flow.visibleTabs.length > 1 && (
        <div className="space-y-2">
          ...
          <div className="-mx-4 px-4 overflow-x-auto no-scrollbar snap-x snap-mandatory sm:overflow-visible sm:mx-0 sm:px-0 border-b border-border/50">
            <div className="flex items-center gap-3 sm:gap-6 sm:flex-wrap whitespace-nowrap">
              {flow.visibleTabs.map((t) => {
                ...
                const mobileLabelMap: Record<Step4SubStep, string> = {
                  look: "Look",
                  essentials: "Essentials",
                  people: "People",
                  interaction: "Interaction",
                ...
                return (
                  <button
                    ...
                    className={`relative pb-2.5 sm:pb-3 -mb-px border-b-2 text-[12px] font-medium transition-colors inline-flex items-center gap-1.5 snap-start shrink-0 ${
                      active
                        ? "border-foreground text-foreground"
                        : "border-transparent text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    <span className="hidden sm:inline">{labelMap[t]}</span>
                    <span className="sm:hidden">{mobileLabelMap[t]}</span>
                    {done && (
                      <Check
                        className={`w-3 h-3 ${
